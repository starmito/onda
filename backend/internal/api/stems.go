...
// exportFilePrefixes lists the filename prefixes generated by Onda's export
// endpoints. Files matching these prefixes are omitted from stem/pitch listings
// because they are mixdowns or exports, not separable stems. The prefixes
// cover:
//   - merge_*   from POST /api/stems/merge
//   - export_*  from POST /api/audio/export
//   - export.mid from POST /api/daw/midi/export
var exportFilePrefixes = []string{"merge_", "export_"}

// exportFileNames lists exact export filenames to omit from stem/pitch listings.
var exportFileNames = map[string]bool{
	"export.mid": true,
}

// isExportFileName reports whether name is a file produced by an export
// endpoint. It is used to hide exported mixdowns from the stem and pitch
// subgroup lists, including legacy exports that were written directly into the
// song directory before the default export directory moved under the data root.
func isExportFileName(name string) bool {
	if exportFileNames[name] {
		return true
	}
	lower := strings.ToLower(name)
	for _, prefix := range exportFilePrefixes {
